  ...
  return (
    <div className="h-screen w-screen bg-gradient-subtle overflow-hidden flex items-center justify-center">
      <div className="w-full max-w-4xl h-full max-h-screen flex flex-col gap-2 p-2">
        ...
        <div className="text-center flex-shrink-0">
          ...
          <div className="flex justify-center items-center mb-1">
            <img 
              ...
              alt="Colormaq Logo" 
              className="h-12 w-auto filter brightness-0 invert dark:brightness-100 dark:invert-0"
            />
          </div>
        ...
        {/* Status Card */}
        <Card className={`p-4 transition-all duration-500 shadow-colormaq flex-shrink-0 ${getStateClasses()}`}>
          <div className="flex flex-col items-center space-y-3">
            <div className="w-12 h-12 flex items-center justify-center">
              {validationState === 'approved' && <CheckCircle className="w-12 h-12 text-success-foreground" />}
              {validationState === 'rejected' && <XCircle className="w-12 h-12 text-error-foreground" />}
              {validationState === 'error' && <ScanLine className="w-12 h-12 text-warning-foreground" />}
              {validationState === 'waiting' && <ScanLine className="w-12 h-12 text-primary animate-pulse" />}
            </div>
        ...
        <div className="grid md:grid-cols-2 gap-3 flex-shrink-0">
          <Card className={`p-4 space-y-2 transition-all duration-300 ${
            !isSerial1Complete ? 'ring-2 ring-primary shadow-lg' : 'bg-muted/30'
          }`}>
            <Label className="text-lg font-semibold flex items-center gap-2">
              <ScanLine className={`w-4 h-4 ${!isSerial1Complete ? 'text-primary animate-pulse' : 'text-muted-foreground'}`} />
              Código 1
              {!isSerial1Complete && <span className="text-xs font-normal text-primary ml-2">(Aguardando...)</span>}
            </Label>
            <div className={`text-lg p-3 text-center font-mono tracking-wider border-2 border-dashed rounded-lg min-h-[50px] flex items-center justify-center break-all ${
          ...
          <Card className={`p-4 space-y-2 transition-all duration-300 ${
            ...
            <Label className="text-lg font-semibold flex items-center gap-2">
              <ScanLine className={`w-4 h-4 ${isSerial1Complete && !serial2 ? 'text-primary animate-pulse' : 'text-muted-foreground'}`} />
              Código 2
              {isSerial1Complete && !serial2 && <span className="text-xs font-normal text-primary ml-2">(Aguardando...)</span>}
            </Label>
            <div className={`text-lg p-3 text-center font-mono tracking-wider border-2 border-dashed rounded-lg min-h-[50px] flex items-center justify-center break-all ${
        ...
        {/* Controls */}
        <div className="flex justify-center flex-wrap gap-2 flex-shrink-0">
          <Button
        ...
        {/* Stats e Quick Info - Compactados em uma única seção com scroll se necessário */}
        <div className="flex-1 min-h-0 overflow-y-auto">
          {validationHistory.length > 0 && (
            <Card className="p-4 mb-3">
              <h3 className="text-lg font-semibold mb-3 text-center">Estatísticas da Sessão</h3>
              <div className="grid md:grid-cols-3 gap-4 text-center">
                <div>
                  <div className="text-xl font-bold text-primary">{validationHistory.length}</div>
                  <div className="text-xs text-muted-foreground">Total de Validações</div>
                </div>
                <div>
          ...
          <Card className="p-4 bg-muted/50">
            <div className="grid md:grid-cols-3 gap-4 text-center">
              <div>
                <h3 className="font-semibold text-success text-sm">APROVADO</h3>
                <p className="text-xs text-muted-foreground">Códigos idênticos</p>
              </div>
              <div>
                <h3 className="font-semibold text-error text-sm">REPROVADO</h3>
                <p className="text-xs text-muted-foreground">Códigos diferentes</p>
              </div>
              <div>
                <h3 className="font-semibold text-warning text-sm">ERRO</h3>
                <p className="text-xs text-muted-foreground">Formato inválido</p>
              </div>
            </div>
          </Card>
        </div>
